...
/** @file
 * @short utility to prepare the structures for the H.265 framer unit test
 */
                       ...
                       struct upump **upump_p)
{
    ...
    /* append an annex B end-of-bitstream NAL so the framer flushes the AU */
    buffer[size++] = 0;
    buffer[size++] = 0;
    buffer[size++] = 0;
    buffer[size++] = 1;
    buffer[size++] = H265NAL_TYPE_EOB << 1;
    buffer[size++] = 1;
    dump_variable("h265_pic", buffer, size);
    uref_free(uref);
}
    ...
    switch (command) {
        case UPIPE_SET_FLOW_DEF: {
            ...
            if (ubase_check(uref_flow_get_headers(flow_def, &headers, &size)))
                dump_variable("h265_headers", headers, size);
            return UBASE_ERR_NONE;
        }
...
int main(int argc, char **argv)
{
    ...
    struct uref *x265_flow_def = uref_pic_flow_alloc_def(uref_mgr, 1);
    assert(x265_flow_def);
    ubase_assert(uref_pic_flow_add_plane(x265_flow_def, 1, 1, 1, "y8"));
    ubase_assert(uref_pic_flow_add_plane(x265_flow_def, 2, 2, 1, "u8"));
    ubase_assert(uref_pic_flow_add_plane(x265_flow_def, 2, 2, 1, "v8"));
    ubase_assert(uref_pic_flow_set_hsize(x265_flow_def, WIDTH));
    ubase_assert(uref_pic_flow_set_vsize(x265_flow_def, HEIGHT));
    struct urational fps = { .num = 25, .den = 1 };
    ubase_assert(uref_pic_flow_set_fps(x265_flow_def, fps));
    ubase_assert(uref_pic_set_progressive(x265_flow_def, true));

    /* x265 pipe */
    struct upipe_mgr *x265_mgr = upipe_x265_mgr_alloc();
    assert(x265_mgr != NULL);
    struct upipe *x265 = upipe_void_alloc(x265_mgr,
                    uprobe_pfx_alloc(uprobe_use(uprobe), UPROBE_LOG_LEVEL,
                                     "x265"));
    assert(x265 != NULL);
    ubase_assert(upipe_x265_set_default_preset(x265, "faster", "zerolatency"));
    ubase_assert(upipe_x265_set_profile(x265, "main"));
    ubase_assert(upipe_set_option(x265, "bitrate", "100"));
    ubase_assert(upipe_set_option(x265, "vbv-bufsize", "100"));
    ubase_assert(upipe_set_option(x265, "vbv-maxrate", "100"));
    ubase_assert(upipe_set_option(x265, "aud", "0"));
    ubase_assert(upipe_set_option(x265, "repeat-headers", "0"));
    ubase_assert(upipe_set_option(x265, "info", "0"));
    ubase_assert(upipe_set_option(x265, "keyint", "1"));
    ubase_assert(upipe_set_flow_def(x265, x265_flow_def));
    ubase_assert(upipe_set_output(x265, sink));

    uref = uref_pic_alloc(uref_mgr, pic_mgr, WIDTH, HEIGHT);
    fill_pic(uref, 0);
    upipe_input(x265, uref_dup(uref), NULL);
    upipe_release(x265);

    uref_free(uref);
    test_free(sink);
    uref_free(x265_flow_def);

    upipe_mgr_release(x265_mgr);
    upipe_x265_cleanup();
