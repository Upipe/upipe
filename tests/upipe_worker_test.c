...
UPIPE_HELPER_UPIPE(linear, upipe, 0);
UPIPE_HELPER_UREFCOUNT(linear, urefcount, linear_free);
UPIPE_HELPER_VOID(linear);
UPIPE_HELPER_OUTPUT(linear, output, flow_def, output_state, requests);

static struct upipe *linear_alloc(struct upipe_mgr *mgr,
                                  struct uprobe *uprobe,
                                  uint32_t signature,
                                  va_list args)
{
    struct upipe *upipe = linear_alloc_void(mgr, uprobe, signature, args);
    assert(upipe);

    linear_init_urefcount(upipe);
    linear_init_output(upipe);
...
static void linear_free(struct upipe *upipe)
{
    upipe_throw_dead(upipe);

    linear_clean_output(upipe);
    linear_clean_urefcount(upipe);
    linear_free_void(upipe);
}

static void linear_input(struct upipe *upipe,
                         struct uref *uref,
                         struct upump **upump_p)
{
    linear_output(upipe, uref, upump_p);
}

static int linear_set_flow_def(struct upipe *upipe, struct uref *flow_def)
{
    ubase_assert(uref_flow_match_def(flow_def, UREF_VOID_FLOW_DEF));
    struct uref *flow_def_dup = uref_dup(flow_def);
    linear_store_flow_def(upipe, flow_def_dup);
    return UBASE_ERR_NONE;
}

static int linear_control(struct upipe *upipe, int cmd, va_list args)
{
    UBASE_HANDLED_RETURN(linear_control_output(upipe, cmd, args));
    switch (cmd) {
        case UPIPE_ATTACH_UPUMP_MGR:
            return UBASE_ERR_NONE;
        case UPIPE_SET_FLOW_DEF: {
            struct uref *flow_def = va_arg(args, struct uref *);
            return linear_set_flow_def(upipe, flow_def);
        }
    }
    return UBASE_ERR_UNHANDLED;
}

static struct upipe_mgr linear_mgr = {
    .refcount = NULL,
    .signature = 0,
    .upipe_alloc = linear_alloc,
    .upipe_input = linear_input,
    .upipe_control = linear_control,
};
...
int main(int argc, char *argv[])
{
    ...
    struct upipe *linear0 =
        upipe_void_alloc_output(remote_source, &linear_mgr,
                                uprobe_pfx_alloc(uprobe_use(uprobe_remote),
                                                 UPROBE_LOG_LEVEL, "lin0"));
    uprobe_throw(uprobe_main, NULL, UPROBE_THAW_UPUMP_MGR);
    upipe_release(linear0);
    ...
    struct upipe *linear1 =
        upipe_void_alloc_output(source, &linear_mgr,
                                uprobe_pfx_alloc(uprobe_use(uprobe_main),
                                                 UPROBE_LOG_LEVEL, "lin1"));
    assert(linear1);
    upipe_release(linear1);

    struct upipe *sink =
        upipe_void_alloc_output(linear1, &sink_mgr,
                                uprobe_pfx_alloc(uprobe_use(uprobe_main),
    ...
    uprobe_notice(logger, NULL, "joining");
    assert(!pthread_join(remote_thread_id, NULL));
    uprobe_notice(logger, NULL, "joined");
    ...
    assert(!pthread_create(&remote_thread_id, NULL, thread, upipe_xfer_mgr));
    uprobe_pthread_assert_set(uprobe_remote, remote_thread_id);
    upipe_work_mgr = upipe_work_mgr_alloc(upipe_xfer_mgr);
    upipe_mgr_release(upipe_xfer_mgr);
    assert(upipe_work_mgr);
    ...
    linear0 =
        upipe_void_alloc(&linear_mgr,
                         uprobe_pfx_alloc(uprobe_use(uprobe_remote),
                                          UPROBE_LOG_LEVEL, "lin0"));
    uprobe_throw(uprobe_main, NULL, UPROBE_THAW_UPUMP_MGR);

    linear1 =
        upipe_void_alloc_output(linear0, &linear_mgr,
                                uprobe_pfx_alloc(uprobe_use(uprobe_remote),
                                                 UPROBE_LOG_LEVEL, "lin1"));
    assert(linear1);
    upipe_release(linear1);
    ...
    struct upipe *worker =
        upipe_work_alloc(upipe_work_mgr,
                         ...
                         uprobe_pfx_alloc(uprobe_use(uprobe_remote),
                                          UPROBE_LOG_LEVEL, "wsrc_x"),
                         WORK_IN_QUEUE, WORK_OUT_QUEUE);
    assert(worker);

    ubase_assert(upipe_set_output(source, worker));

    sink =
        upipe_void_alloc_output(worker, &sink_mgr,
                                uprobe_pfx_alloc(uprobe_use(uprobe_main),
                                                 UPROBE_LOG_LEVEL, "sink"));
    ...
    ubase_assert(upipe_set_output(linear0, worker));
    upipe_release(linear0);
    upipe_release(worker);
