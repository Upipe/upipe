...
/** @internal @This is the private context of an s337d pipe. */
struct upipe_s337d {
...
UPIPE_HELPER_UPIPE(upipe_s337d, upipe, UPIPE_S337D_SIGNATURE)
UPIPE_HELPER_UREFCOUNT(upipe_s337d, urefcount, upipe_s337d_free)
UPIPE_HELPER_VOID(upipe_s337d)
 ...
 * @param upump_p reference to pump that generated the buffer
 */
static void upipe_s337d_work(struct upipe *upipe, struct upump **upump_p)
{
    struct upipe_s337d *upipe_s337d = upipe_s337d_from_upipe(upipe);
    while (upipe_s337d->next_uref != NULL) {
        if (upipe_s337d->next_frame_size == -1) {
            size_t dropped = 0;
            bool ret = upipe_s337d_scan(upipe, &dropped);
            upipe_s337d_consume_uref_stream(upipe, dropped);
            if (!ret)
                return;
        }

        if (upipe_s337d->next_frame_size == -1) {
            upipe_s337d_parse_preamble(upipe);
            if (upipe_s337d->next_frame_size == -1)
                return; /* not enough data */
        }

        if (!upipe_s337d_check_frame(upipe))
            return;

        if (upipe_s337d->next_frame_discard) {
            upipe_s337d_consume_uref_stream(upipe,
                                            upipe_s337d->next_frame_size);
        } else {
            upipe_s337d_sync_acquired(upipe);
            upipe_s337d_output_frame(upipe, upump_p);
        }
        upipe_s337d->next_frame_size = -1;
        upipe_s337d->next_frame_discard = true;
    }
 ...
 * @param upump_p reference to pump that generated the buffer
 */
static void upipe_s337d_input(struct upipe *upipe, struct uref *uref,
                              struct upump **upump_p)
{
    upipe_s337d_append_uref_stream(upipe, uref);
    upipe_s337d_work(upipe, upump_p);
}
...
/** @internal @This processes control commands on a s337d pipe.
 *
 * @param upipe description structure of the pipe
 ...
 * @return an error code
 */
static int upipe_s337d_control(struct upipe *upipe, int command, va_list args)
{
    UBASE_HANDLED_RETURN(upipe_s337d_control_output(upipe, command, args));

    switch (command) {
        case UPIPE_SET_FLOW_DEF: {
            struct uref *flow_def = va_arg(args, struct uref *);
            return upipe_s337d_set_flow_def(upipe, flow_def);
        }
        default:
...
static void upipe_s337d_free(struct upipe *upipe)
{
    ...
    upipe_s337d_clean_output(upipe);
    upipe_s337d_clean_flow_def(upipe);
    upipe_s337d_clean_sync(upipe);

    upipe_s337d_clean_urefcount(upipe);
    upipe_s337d_free_void(upipe);
}

/** module manager static descriptor */
static struct upipe_mgr upipe_s337d_mgr = {
    .refcount = NULL,
    .signature = UPIPE_S337D_SIGNATURE,

    .upipe_alloc = upipe_s337d_alloc,
    .upipe_input = upipe_s337d_input,
    .upipe_control = upipe_s337d_control,
