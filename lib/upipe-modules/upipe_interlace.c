                                 ...
                                 const char *chroma)
{
    ...
    // map
    ret = uref_pic_plane_read(top, chroma, 0, 0, -1, -1, &t_in);
    if (unlikely(!ubase_check(ret))) {
        upipe_warn_va(upipe, "fail to read %s plane", chroma);
        return ret;
    }
    ret = uref_pic_plane_read(bottom, chroma, 0, 0, -1, -1, &b_in);
    if (unlikely(!ubase_check(ret))) {
        upipe_warn_va(upipe, "fail to read %s plane", chroma);
    ...
    ret = ubuf_pic_plane_write(ubuf, chroma, 0, 0, -1, -1, &out);
    if (unlikely(!ubase_check(ret))) {
        upipe_warn_va(upipe, "fail to write %s plane", chroma);
                                        ...
                                        struct uref *flow_def)
{
    ...
    if (!uref_pic_check_progressive(flow_def)) {
        bool input_tff = uref_pic_check_tff(flow_def);
        if (input_tff != upipe_interlace->tff) {
            upipe_err_va(upipe, "input interlacing mismatch, got %s need %s",
                         input_tff ? "tff" : "bff",
 ...
 * @return an error code
 */
static int _upipe_interlace_set_tff(struct upipe *upipe, bool tff)
{
    struct upipe_interlace *upipe_interlace = upipe_interlace_from_upipe(upipe);
 ...
 * @return an error code
 */
static int _upipe_interlace_get_tff(struct upipe *upipe, bool *tff)
{
    struct upipe_interlace *upipe_interlace = upipe_interlace_from_upipe(upipe);
    if (tff)
 ...
 * @return an error code
 */
static int _upipe_interlace_set_drop(struct upipe *upipe, bool drop)
{
    struct upipe_interlace *upipe_interlace = upipe_interlace_from_upipe(upipe);
 ...
 * @return an error code
 */
static int _upipe_interlace_get_drop(struct upipe *upipe, bool *drop)
{
    struct upipe_interlace *upipe_interlace = upipe_interlace_from_upipe(upipe);
    if (drop)
                                   ...
                                   va_list args)
{
    switch (command) {
        ...
        case UPIPE_INTERLACE_SET_TFF: {
            ...
            return _upipe_interlace_set_tff(upipe, tff != 0);
        }
        case UPIPE_INTERLACE_GET_TFF: {
            UBASE_SIGNATURE_CHECK(args, UPIPE_INTERLACE_SIGNATURE);
            bool *tff = va_arg(args, bool *);
            return _upipe_interlace_get_tff(upipe, tff);
        }
        case UPIPE_INTERLACE_SET_DROP: {
            UBASE_SIGNATURE_CHECK(args, UPIPE_INTERLACE_SIGNATURE);
            int drop = va_arg(args, int);
            return _upipe_interlace_set_drop(upipe, drop != 0);
        }
