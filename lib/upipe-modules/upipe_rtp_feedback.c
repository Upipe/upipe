...
static void upipe_rtpfb_timer(struct upump *upump)
{
    ...
    ulist_delete_foreach(&upipe_rtpfb->queue, uchain, uchain_tmp) {
        ...
        if (now - cr_sys <= upipe_rtpfb->latency * UCLOCK_FREQ / 1000)
            break;

        upipe_verbose_va(upipe, "Output seq %hu after %"PRIu64" clocks", seqnum, now - cr_sys);
        ...
        const uint16_t seqnum, struct uref *next)
{
    ...
    upipe_dbg_va(upipe, "Repaired %"PRIu64" > %hu > %"PRIu64" -diff %hu",
...
static int upipe_rtpfb_set_option(struct upipe *upipe, const char *k, const char *v)
{
    ...
    if (!k || !v) {
        ...
        upipe_rtpfb->latency = atoi(v);
        upipe_dbg_va(upipe, "Setting latency to %"PRIu64" msecs",
                upipe_rtpfb->latency);
    } else
        return UBASE_ERR_INVALID;
