                                ...
                                struct upump **upump_p)
{
    ...
    if (upipe_grid_in->frame_latency) {
        ...
        if (ubase_check(uref_pic_flow_get_fps(flow_def, &fps)) && fps.num) {
            latency =
                upipe_grid_in->frame_latency * fps.den * UCLOCK_FREQ / fps.num;
        } else if (ubase_check(uref_sound_flow_get_rate(flow_def, &rate)) &&
                   uref_sound_flow_get_samples(flow_def, &samples) && rate) {
            latency =
                upipe_grid_in->frame_latency * samples * UCLOCK_FREQ / rate;
        }
        pts += latency;
    }
 ...
 * @return an error code
 */
static int upipe_grid_in_get_frame_latency_real(struct upipe *upipe,
                                                unsigned *nb)
{
    struct upipe_grid_in *upipe_grid_in = upipe_grid_in_from_upipe(upipe);
    if (nb)
        *nb = upipe_grid_in->frame_latency;
    return UBASE_ERR_NONE;
}
 ...
 * @return an error code
 */
static int upipe_grid_in_set_frame_latency_real(struct upipe *upipe,
                                                       unsigned nb)
{
    struct upipe_grid_in *upipe_grid_in = upipe_grid_in_from_upipe(upipe);
    upipe_grid_in->frame_latency = nb;
    return UBASE_ERR_NONE;
}
                                      ...
                                      int command, va_list args)
{
    ...
    if (command > UPIPE_CONTROL_LOCAL &&
        ubase_get_signature(args) == UPIPE_GRID_IN_SIGNATURE) {
        switch (command) {
            case UPIPE_GRID_IN_GET_FRAME_LATENCY: {
                UBASE_SIGNATURE_CHECK(args, UPIPE_GRID_IN_SIGNATURE);
                unsigned *nb = va_arg(args, unsigned *);
                return upipe_grid_in_get_frame_latency_real(upipe, nb);
            }
            case UPIPE_GRID_IN_SET_FRAME_LATENCY: {
                UBASE_SIGNATURE_CHECK(args, UPIPE_GRID_IN_SIGNATURE);
                unsigned nb = va_arg(args, unsigned);
                return upipe_grid_in_set_frame_latency_real(upipe, nb);
            }
        }
    }
