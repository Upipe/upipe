                            ...
                            struct audio_ctx *ctx)
{
    ...
    if (upipe_sdi_dec->debug) {
        ...
        for (int i = 0; i < 1; i++) {
            uint16_t prev_clk = upipe_sdi_dec->audio_debug.prev_clk[i];
            uint64_t prev_line = upipe_sdi_dec->audio_debug.prev_line[i];
            ...
            upipe_notice_va(upipe, "line: %d, prev clk: %d, clk: %d, diff: %d",
                    line_num, prev_clk, clock, diff);

            upipe_sdi_dec->audio_debug.prev_clk[i] = clock;
            upipe_sdi_dec->audio_debug.prev_line[i] = line;
        ...
        uint16_t *r, size_t hsize)
{
    while (hsize > S291_HEADER_SIZE + S291_FOOTER_SIZE) {
        ...
        if (did == S291_AFD_DID && sdid == S291_AFD_SDID && dc == 8) {
            ...
            uint16_t cdp_identifier = ((r[0] & 0xff) << 8) | (r[1] & 0xff);
            ...
            uint16_t *ccdata_section = &r[S334_CDP_HEADER_LEN + (timecode ? S334_CDP_TIMECODE_LEN : 0)];
            if ((ccdata_section[0] & 0xff) != S334_CDP_CCDATA_ID) {
                             ...
                             struct upump **upump_p)
{
    ...
    if (ubuf_sound && last_line == f->height) {
        ...
        if (samples_received < expected) {
            ...
            size_t offset = wrong_samples * UPIPE_SDI_CHANNELS_PER_GROUP * UPIPE_SDI_CHANNELS_PER_GROUP;
